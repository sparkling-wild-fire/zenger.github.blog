...
1. break设置断点
   ...
   - `b test`   在函数入口处设置断点，如果这个函数有多个，会给所有同名函数（不论参数是否相同）都设置断点
     - 这其实是非常有用的，因为我们又是只知道执行某个函数会出错，而不知到是子类还是父类的函数出错
     - 如图所示，该命令设置了两个断点
...
4. 附加到进程：适用于程序已经启动，如果我们用gdb去启动这个程序，那调试的也是另一个新的程序
    - gdb attach <pid>
    - gdb --pid <pid>
    - 以我们公司为例，一个节点master_node要对接一个外部节点out_node，此时master_node节点已经启动了，进程号为`1342`,此时out_node节点要通过一个函数Fun()接入进来，但发现接入的时候出错了，就需要以下操作：
      - `gdb --pid 1342`
      - `b Func:45` (45表示行数，从日志看是这行抛出了异常) 是冒号嘛？？？
      - `run`
...
当函数执行到test_member內部时，利用backtrace命令查看：
