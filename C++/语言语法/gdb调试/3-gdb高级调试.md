...
有时我们想向前向后执行代码，比如我们想跳转到第12行，需要如此执行：
...
注意：多线程程序不支持这种回退命令。因为多线程共享进程的虚拟地址空间，本线程回退后，会导致其他线程访问内存时出错。
...
如在gdb里调用自己写的函数（调试的程序可以不携带调试信息），然后在调用的函数内部打对应的断点，然后检查输出的返回值。(tip:单元测试建议使用gtest)
...
如algoserverForwin就只支持realse版本，可以通过vs2015生成调试文件。
